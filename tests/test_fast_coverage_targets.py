...
def test_campaign_execute_methods_and_print_summary(monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    orchestrator.log = SimpleNamespace(warning=lambda *_args, **_kwargs: None)
    orchestrator.validator = SimpleNamespace(validate=lambda text: SimpleNamespace(is_valid=True, confidence=0.9, to_dict=lambda: {"t": text}))
    orchestrator.search_space = SimpleNamespace(get_coverage_report=lambda: {"coverage": 1})
    ...
    monkeypatch.setattr("kryptos.pipeline.k4_campaign.recover_key_by_frequency", lambda _ct, _kl, top_n=1: ["ABCD"])
    monkeypatch.setattr("kryptos.pipeline.k4_campaign.vigenere_decrypt", lambda _ct, _k: "DECRYPTED")
    monkeypatch.setattr(
        "kryptos.pipeline.k4_campaign.solve_columnar_permutation_exhaustive",
        lambda _ct, _period: ([0, 1], 0.4),
    )
    monkeypatch.setattr(
        "kryptos.pipeline.k4_campaign.solve_columnar_permutation_simulated_annealing_multi_start",
        lambda _ct, _period: ([1, 0], 0.6),
    )
