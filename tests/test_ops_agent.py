...
def test_ops_execute_parallel_success():
    """Test parallel execution with successful jobs."""

    class FastHypothesis:
        def generate_candidates(self, ciphertext, limit=10):
            Candidate = type("Candidate", (), {"score": -330.0, "plaintext": "FAST"})
            return [Candidate() for _ in range(3)]

    agent = OpsAgent(config=OpsConfig(max_workers=2))
    jobs = [
        {"name": "job1", "class": FastHypothesis},
        {"name": "job2", "class": FastHypothesis},
    ]

    results = agent.execute_parallel(jobs, "ABCDEFGHIJ")

    assert len(results) == 2
    assert all(r.success for r in results)
    assert all(r.candidates_count == 3 for r in results)


def test_ops_execute_parallel_with_failure():
    """Test parallel execution with mixed success/failure."""

    class SuccessHypothesis:
        def generate_candidates(self, ciphertext, limit=10):
            Candidate = type("Candidate", (), {"score": -330.0, "plaintext": "OK"})
            return [Candidate()]

    class FailHypothesis:
        def generate_candidates(self, ciphertext, limit=10):
            raise RuntimeError("Job failed")

    agent = OpsAgent(config=OpsConfig(max_workers=2))
    jobs = [
        {"name": "success_job", "class": SuccessHypothesis},
        {"name": "fail_job", "class": FailHypothesis},
    ]
    ...
    assert "Job failed" in failed_results[0].error


def test_ops_execute_parallel_with_params():
    """Test parallel execution with job-specific parameters."""

    class ParamHypothesis:
        def __init__(self, custom_param=None):
            self.custom_param = custom_param

        def generate_candidates(self, ciphertext, limit=10):
            Candidate = type("Candidate", (), {"score": -330.0, "plaintext": f"PARAM_{self.custom_param}"})
            return [Candidate()]

    agent = OpsAgent()
    jobs = [
        {"name": "param_job1", "class": ParamHypothesis, "params": {"custom_param": "VALUE1"}},
        {"name": "param_job2", "class": ParamHypothesis, "params": {"custom_param": "VALUE2"}},
    ]
