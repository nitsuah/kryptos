...
| Physical-geometry schema | ✅ Complete | `kryptos.k4.physical_geometry` — typed landing spot for sculpture measurements; every field `None` until a cited source supplies it. Only the tableau's back-only reading direction is confirmed (CIA's own page); compass bearing and lodestone deflection remain open (Phase 8). |
...
| Legacy hypothesis-runner utility | ⚠️ Bug fixed, still unused | `kryptos.k4.hypothesis_runner.run_hypothesis_search` — a reusable "run a `Hypothesis`, print top candidates, save an artifact" utility. Found during this table's own audit: its default `ciphertext` parameter was a **hardcoded, truncated 74-character K4 fragment**, silently missing the BERLIN/CLOCK crib positions entirely (positions 63-73 don't exist in a 74-char string) — a landmine for any future caller relying on the default. Its only current caller (`tests/smoke/test_fast_coverage_targets.py`) always overrides `ciphertext` explicitly, so this was never actually exercised. Fixed to import the canonical `physical_grid.K4` instead of hardcoding its own copy — the same fix pattern as the 15-file K4-constant consolidation earlier this session. Still has zero real callers; kept rather than deleted since it's a small, genuinely reusable utility, not obviously dead weight. |
