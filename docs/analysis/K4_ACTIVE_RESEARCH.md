...
| "Shadow of the word" as a computed shadow angle — World Clock topper rotation (full 0-23 sweep) and real solar azimuth at CIA HQ (Phase 7) | NULL RESULT | `kryptos.k4.solar_geometry` — solar-position algorithm verified against known reference points; topper-rotation hypothesis honestly reduced to a full-range sweep after finding every sourced timestamp pair is vacuously co-phased. **1,244,160 candidates** (topper) + **108,864** (solar bearing route directions). Zero hits. Artifacts: `K4_GEOMETRY_COMBINED_TOPPER_ROTATION_NULL.json`, `K4_GEOMETRY_COMBINED_SOLAR_BEARING_NULL.json`. |
| World Clock's 146-city list as keyed-alphabet/numeric-offset source (Phase 7) | NULL RESULT | `kryptos.k4.world_clock_cities` — 104 individually-sourced city names (read directly off the sculpture's plates via 5 Wikimedia Commons photographs, ~17 of 24 segments; complete 146-name list still unavailable) as keyed alphabets (**9,720 + 112,320 candidates**, text-sourced then photograph-expanded) plus 3 sourced structural counts (146/147/24) as rotation offsets (**155,520 candidates**). Zero hits. Artifacts: `K4_WORLD_CLOCK_CITIES_NULL.json`, `K4_GEOMETRY_COMBINED_WORLDCLOCK_NULL.json`. |
...
| Solar-azimuth (hypothesis B) route directions | `order_names=GEO_BEARING_ORDER_NAMES` (now including 4 solar-shadow bearing pairs) | 108,864 | Null |
| Topper full-rotation sweep (hypothesis A) | `rotation_offsets=range(24)` (full axis, not just priority values) | 1,244,160 | Null |

Artifacts: `K4_GEOMETRY_COMBINED_SHAPECHANGING_NULL.json`, `K4_GEOMETRY_COMBINED_SHAPECHANGING_GEO_NULL.json`, `K4_3LAYER_GEOMETRIC_SHAPECHANGING_NULL.json`, `K4_GEOMETRY_COMBINED_SOLAR_BEARING_NULL.json`, `K4_GEOMETRY_COMBINED_TOPPER_ROTATION_NULL.json`.
...
- **List massively expanded via direct primary-source reading, not a text summary.** The clock is a permanently-photographed public sculpture — rather than accept "no source lists all the names," five Wikimedia Commons photographs of the actual engraved plates (`Weltzeituhr_Detail_Alexanderplatz.jpg`, `Weltzeituhr.jpg`, `Die_Urania-Weltzeituhr_am_Alexanderplatz.jpg`, `DSC_3226_Urania-Weltzeituhr_Berlin_I.jpg`, `Weltzeituhr,_Berlin_(15910006062).jpg`) were opened directly in-browser and read visually, transcribing every legible plate rather than relying on any secondary description. This covers roughly 17 of the 24 segments — **104 individually confirmed names** in `kryptos.k4.world_clock_cities.CONFIRMED_CITIES`, up from 9. Still not the complete 146 (Japan/Korea/Australia/NZ, the Pacific/Hawaii zones, and one Middle East/North Africa gap weren't found legibly photographed), but a large, directly-verified jump rather than a stalled "no source available."
...
| Confirmed city names as keyed alphabets (expanded, photograph-sourced) | 104 confirmed names × 3 grid widths × 2 priority clock states | 112,320 | Null |
...
**Grand total across Phase 7's real-K4 sweeps: 2,269,944 candidates (plus the 30-artifact, 11-vector cross-scan), zero breakthroughs, zero cross-vector consensus.**
