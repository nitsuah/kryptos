...
| World Clock's 148-city list as keyed-alphabet/numeric-offset source (Phase 7) | NULL RESULT | `kryptos.k4.world_clock_cities` — 9 individually-sourced city names (complete list unavailable, not fabricated) as keyed alphabets (**9,720 candidates**) plus 2 sourced structural counts as rotation offsets (**103,680 candidates**). Zero hits. Artifacts: `K4_WORLD_CLOCK_CITIES_NULL.json`, `K4_GEOMETRY_COMBINED_WORLDCLOCK_NULL.json`. |
...
| Sourced structural counts as rotation offsets | `{148 mod 24, 24 mod 24}` = `{4, 0}` | 103,680 | Null |
