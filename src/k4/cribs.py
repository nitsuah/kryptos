...
def annotate_cribs(ciphertext: str, crib_plain_to_cipher: Dict[str, str], one_based: bool = True) -> List[Dict[str, object]]:
    """Annotate crib placements.

    Args:
        ciphertext: Raw or normalized ciphertext.
        crib_plain_to_cipher: Mapping of known plaintext crib -> expected cipher segment released by Sanborn.
        one_based: Whether positions should be reported one-based (default True per sculpture convention).

    Returns:
        List of annotation dicts with fields:
          plaintext, expected_cipher, expected_positions (tuple or None), found_positions (list[int]), alignment_ok (bool)
    """
