...
class PipelineExecutor:  # DEPRECATED: see module docstring
    ...
    def run(self, ciphertext: str) -> dict[str, Any]:
        ...
        for stage in self.config.ordering:
            ...
            cps = len(candidates) / duration if duration > 0 else 0.0
            pps = len(pruned) / duration if duration > 0 else 0.0
        ...
        with open(csv_path, 'w', newline='', encoding='utf-8') as csvfh:
            ...
            writer.writerow(['stage', 'rank', 'score', 'crib_bonus', 'text_prefix'])
